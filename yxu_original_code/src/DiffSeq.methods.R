...
###. merge samples: newer with faster implementation and less memory demanding
...
merge.samples <- function(rootpath,mappedFiles,outpath) {
  ...
  for (chr in names(out)) {
    ...
    save.ws(ws="joinSamples", what=obj.name,chr=chr, outpath=outpath)
    cat("DONE!\n")
  }
  cat("writting n.reads and count.table to workspace...\n")
  cat("",tabsps("n.reads"))
  save(n.reads, file=file.path(get.wsoption.path("joinSamples"),"nreads.RData"))
  cat("DONE!\n")
  cat("",tabsps("count.tbs"))
  save(count.tbs, file=file.path(get.wsoption.path("joinSamples"),"countdistr.RData"))
  cat("DONE!\n")
}
...
join.samples.chr <- function(chr, x, ..., by=100,filePrefix="JoinSamples/JoinedSamples") {

  arglist <- lapply(list(x,...), function(x) subset(x, subset=(V1 == chr)))
  filename <- paste(filePrefix, chr, "RData",sep=".")
  assign(paste("s",chr, sep="."), do.call(join.samples, args=c(arglist,by=by)))
...
para.joinSamples <- function(x, ..., n.clusters=2, chrs=NULL, by=100,
                             filePrefix="JoinSamples/JoinedSamples") {
...
events.score <- function(res, events.res, rownames, sample.label) {
  ## so called event probability
  events.res <- as.data.frame(events.res)
  sel.nonull <- !unlist(lapply(res, is.null))
  
  p3 <- c(unlist(lapply(res, get.p)))
  w3.res <- as.data.frame(lapply(res, get.w3)[sel.nonull])
  
  e.res <- vector('list', ncol(events.res))
  ##patts <- as.data.frame(pattern.design(events.res, sample.label))
  patts <- pattern.design(events.res, sample.label)

  for (i in seq(ncol(events.res))) {
    e.res[[i]] <- event.prob(w3.res, event=patts[,i],p=p3)
  }
...
events.cutoff <- function(res, e.res, cutoff, method = c("beta", "BF"),find.range= c(-20,20)) {
...
choose.events.cutoff <- function(res, e.res, cutoff, method=c("BF","beta")) {
  method <- match.arg(method)
...
win.select <- function(e.res, e.cut){
  ...
  ## e.cut <- apply(events.res, 2,
  ##                function(x, each, p, p.cut) event.prob.cut( n.win,
  ##                                                           event=rep(x, each=each),
  ##                                                           p=p,
  ##                                                           p.cut=p.cut),
  ##                each=each,
  ##                p=p3,
                            ...
                            cut.1st=c(3000,2000))
{
  ...
  else {wins.sel <- pattern.events(wins.sel, events.test)
...
cut.wins <- function(wins.df, h, cutoff.cat=3000, cutoff.last=2000)
{
  ...
  res    
  ## wins.cut.list <- tapply(wins, INDEX=list(cat=wins.cat.2),FUN=`[`,simplify = F)
  ## wins.cut.list
...
event.allPossiblePatterns <- function(smplabel, smpselect) {
  alabel <- smplabel[smpselect]
...
event.prob <- function(x, event=c(0,0,0,1,1,1,1,1,1), p=rep(0.33,9), k=1e-36)
  ## assume x is in w3.res format as above
  ## p is the p3
{
                        ...
                        src.file="")
{
  ...
  if(ncore>1) n.wins.cut <- as.numeric(cut(seq(n.wins), n.core)) else n.wins.cut <- rep(1, n.wins)
...
## test.effect.limma <- function(wins, psig, contr= cbind(c(0,1,1),c(0,1,-1))) {
##   dat <- psig[wins$Win,]
## }

## get.testComponents <- function(test.effect, idx=c(2,3,11,12,13,14,15)) {
##   res <- t(as.data.frame(lapply(test.effect, function(x) x[idx])))
##   row.names(res) <- names(test.effect)
##   res
## }
                               ...
                               out.path="Reports")
{
  ...
  if(method == "qvalue") {
    require(qvalue)
    q.sites <- qvalue(sites.data[,pvalue.colname], gui=F)
    out.sites.adj <- cbind(sites.data, qvalue=q.sites$qvalues)
...
compute.qvalues <- function(pvalues,...) {
  require(qvalue)
  qvalue(pvalues, gui=F,...)$qvalues
}

select.sig.sites <- function(flat.test.report, contrast.objs, method.sel="fdr", cutoff.p.sel=0.05,
                                    ...
                                    )
{
  ##browser()
  ws <- 'diffTest'
  ##winsize <- get.wsoption.para('winsize')
  if(missing(path)) path <- get.ws.path(ws)
  if(missing(chrs)) chrs <- get.ws.chrs(ws)
  report.flat <- NULL
  for (chr in chrs) {
    load.diffTestData(path, chr)
                              ...
                              )
{
  ...
  if(which == "root") diffseq.options.ws$ws.root
  else if(which == "mixPoisson") diffseq.options.ws$mixpos.path
  else if(which == "pattRecog") diffseq.options.ws$pattrecog.path
  else if(which == "diffTest") diffseq.options.ws$difftest.path
  else if(which == "joinSamples") diffseq.options.ws$joinsample.path
  else if(which == "log") diffseq.options.ws$log.path
  else if(which == "report") diffseq.options.ws$report.path
  else if(which == "src") diffseq.options.ws$src.path
  else stop("Not a valid option: ", which)
}

get.wsoption.para <- function(which=c("winsize")) {
  which <- match.arg(which)
...
{
  which <- match.arg(which)
  if(file.exists(get.wsoption.path("root"))) {
    ws.root <- get.wsoption.path("root")
  } else if(file.exists(paste("..",get.wsoption.path("root"), sep="/"))) {
    ws.root <- paste("..",get.wsoption.path("root"), sep="/")
  }
  if(which == "root") ws.root
  else paste(ws.root, get.wsoption.path(which), sep="/")
}
...
get.winsize <- function(path) {
  if(missing(path)) winsize.path <- get.ws.path("joinSamples")
...
get.ws <- function(ws=c("mixPoisson","pattRecog","diffTest","joinSamples","log","report"),
                   what,
                   chr,
                   ws.path)
{
  ...
  ws <- match.arg(ws)
  if(ws == "mixPoisson") {
    if(missing(chr)) chr <- get('chr', pos=parent.frame(n=1))
    if(missing(ws.path)) ws.path <- get.ws.path("mixPoisson")
    load.mixPoiData(ws.path, chr)
    for (eachwhat in what) {
      ##output.list <- list(output.list, get(eachwhat))
      assign(eachwhat, get(eachwhat), pos=parent.frame(n=1))
    }
  }
  else if(ws == "pattRecog") {
    if(missing(chr)) chr <- get('chr', pos=parent.frame(n=1))
    if(missing(ws.path)) ws.path <- get.ws.path("pattRecog")
    load.pattRecogData(ws.path, chr)
    for (eachwhat in what) {
      assign(eachwhat, get(eachwhat), pos=parent.frame(n=1))
    }
  }
  else if(ws == "report") {
    if(missing(ws.path)) ws.path <- get.ws.path("report")
    load.reportData(ws.path)
    for (eachwhat in what) {
      assign(eachwhat, get(eachwhat), pos=parent.frame(n=1))
    }
  }
...
get.reads.fromJoinedSamples <- function(inputPath, threshold=1) {
  
  reads.datafile <- paste(inputPath,
                          paste("JoinedSamples",chr,"RData", sep="."),
                          sep="/")
  lf <- load(reads.datafile)
  s.cols <- grep("s[0-9]*",names(get(lf)))
  n.sample <- length(s.cols)

  assign("s.cols", s.cols, pos = parent.frame(n=1))
  assign("reads", get(lf)[apply(get(lf)[,s.cols,drop=F], 1, max) > threshold,], pos=parent.frame(n=1))
  assign("reads.poi", reads$poi, pos=parent.frame(n=1))
  assign("reads.names", names(reads)[s.cols], pos=parent.frame(n=1))
}

get.mappedCountTab <- function(inPath=NULL) {
  if(is.null(inPath)) {
    inPath <- get.wsoption.path("joinSamples")
  }
  cntab.data.file <- paste(inPath, "countdistr.RData", sep="/")
...
compute.pmeans <- function(ws.mixPois.path) {
  ##if(missing(ws.mixPois.path)) ws.mixPois.path <- get.wsoption.path("mixPoisson")
  ## load.mixPoiData(input.mixPois.path)
  if(!missing(ws.mixPois.path)) load.mixPoiData(ws.mixPois.path)
  res <- res[!unlist(lapply(res, is.null))]

  ##lambda2.res <- as.data.frame(lapply(res, get.postLogLambda2, log=F))
  ##row.names(lambda2.res) <- reads.poi
  ##names(lambda2.res) <- reads.names

  ##pmean.res <- as.data.frame(lapply(res, get.postLogLambda3, log=F))
  ...
  assign("pmeans", pmeans, pos = parent.frame(n=1))
  assign("pmeans.norm", pmeans.norm, pos = parent.frame(n=1))
  assign("pmeans.bgnorm.norm", pmeans.bgnorm.norm, pos = parent.frame(n=1))
  assign("pmeans.bgcorrect.norm", pmeans.bgcorrect.norm, pos = parent.frame(n=1))
                      ...
                      chr)
{
  ...
  if(missing(ws.pattRecog.path)) ws.pattRecog.path <- get.ws.path("pattRecog")
  chr.env <- parent.env(environment())
  if(missing(chr)) chr <- get("chr", envir=chr.env, inherits = TRUE)
  load(paste(ws.pattRecog.path, paste("pattRecog_wins",chr,"RData", sep="."), sep="/"))
  for(eachwhich in which) {
    assign(eachwhich, get(eachwhich), pos=parent.frame(n=1))
  }
}

save.ws.diffTest <- function(outPath, what, chr, pf.n=1) {
  if(missing(outPath)) outPath <- get.ws.path("diffTest")
  ## chr.env <- parent.env(environment())
  ## if(missing(chr)) chr <- get("chr", envir=chr.env, inherits = TRUE)
  save(list=what,
       file =file.path(outPath, paste("difftest", chr, "RData", sep=".")),
       envir=parent.frame(n=pf.n))
}


save.ws.mixPoisson <- function(outPath, what, chr, pf.n=1) {
  if(missing(outPath)) outPath <- get.ws.path("mixPoisson")
  save(list=what,
       file=file.path(outPath, paste("mixPoi",chr,"RData", sep=".")),
       envir=parent.frame(n=pf.n)
       )
}
...
save.ws.pattRecog <- function(outPath, what, chr, pf.n=1) {
  ...
  save(list=what,
       file=file.path(outPath, paste("pattRecog_wins",chr,"RData", sep=".")),
       envir=parent.frame(n=pf.n)
       )
}
...
save.ws <- function(ws=c("diffTest","mixPoisson","pattRecog","report", "joinSamples"),
                    what,
                    outpath,
                    chr=NULL,
                    pf.n=2) {
  ws <- match.arg(ws)
  if(ws == "diffTest") {
    save.ws.diffTest(outpath, what, chr, pf.n=pf.n)}
  else if(ws == "mixPoisson") {
    save.ws.mixPoisson(outpath, what, chr, pf.n) }
  else if(ws == "pattRecog") {
    save.ws.pattRecog(outpath, what, chr, pf.n)}
  else if(ws == "report") {
    save.ws.report(outpath, what, pf.n)}
  else if(ws =="joinSamples") {
    save.ws.joinSamples(outpath, what, chr, pf.n)}
  else stop("Not a valid ws option!")
}
...
load.mappedData <- function(smpStr, from=c("R","file")) {
  ##browser()
  from <- match.arg(from)
  ...
  assign("dataload", dataload, pos=parent.frame(n=1))
  assign("n.reads", n.reads, pos=parent.frame(n=1))
}

load.mixPoiData <- function(from, chr) {
  if(missing(chr)) chr <- get('chr', pos=parent.frame(n=1))
  if(missing(from)) from <- get.ws.path('mixPoisson')
  load(paste(from,paste("mixPoi",chr,"RData", sep="."), sep="/"), parent.frame(n=1))
}

load.pattRecogData <- function(from, chr) {
  if(missing(chr)) chr <- get('chr', pos=parent.frame(n=1))
  if(missing(from)) from <- get.ws.path('mixPoisson')
  load(paste(from, paste("pattRecog_wins",chr,"RData", sep="."), sep="/"), parent.frame(n=1))
}

load.diffTestData <- function(from, chr) {
  if(missing(chr)) chr <- get('chr', pos=parent.frame(n=1))
  if(missing(from)) from <- get.ws.path('diffTest')
  load(paste(from, paste("difftest",chr,"RData", sep="."), sep="/"), parent.frame(n=1))
}

load.reportData <- function(from) {
  if(missing(from)) from <- get.ws.path('report')
  load(paste(from, "report.RData", sep="/"), parent.frame(n=1))
}
                        ...
                        )
{
  ...
  if(which != "full") {
    files <- list.files(get.ws.path(which))
    if(which == "mixPoisson") filestrs <- grep("mixPoi.chr[0-9MXY]*.RData", files, v=T)
    else if(which == "pattRecog") filestrs <- grep("pattRecog_wins.chr[0-9MXY]*.RData", files, v=T)
    else if(which == "diffTest") filestrs <- grep("difftest.chr[0-9MXY]*.RData", files, v=T)
    else if(which == "joinSamples") filestrs <- grep("JoinedSamples.chr[0-9MXY]*.RData", files, v=T)
  } else if(which =="full") {
    filestrs <- NULL
  } else stop("Not a valid option: ", which)
  out <- sapply(filestrs, function(x) {
...
get.sharedSource <- function(url) {
  require(RCurl)
  src <- getURL(url)
  ##eval(parse(text = src,), parent.frame(n=1))
  eval(parse(text = src), .GlobalEnv)
}

get.sharedSrc <- function(str) {
  pathstr <- "http://dl.dropbox.com/u/10421230/dpt/current/"
  urlStr <- paste(pathstr, str, sep="")
  get.sharedSource(urlStr)
}
...
rangedSites <- function(sites, winsize) {
  sites.win <- RangesList(sapply(
                                 by(sites,
                                    sites$pattern,
                                    function(x) IRanges(start=as.integer(x$Win),
                                                        width=winsize),
                                    simplify=F),
                                 function(x) x))
  ...
  bounds.site <- ddply(sites,
                       c("pattern","ID"),
                       function(x) {c(start=min(x$Win), end=max(x$Win)+winsize-1)})
  
  sites.site <- RangesList(sapply(
                                  by(bounds.site,
                                     bounds.site$pattern,
                                     function(x) IRanges(start=x$start,end=x$end),
                                     simplify=F),
                                  function(x) x))
...
pattSummary.2dataframe <- function(.out) {
  ...
  names(out) <- c(names(out)[-(ncol(out)-(0:3))],c("N","best.cutoff","best.valid","best.pattern"))
  out
}
...
clean.sitePatt <- function(patt, sites, e.TF) {
  ### sites is IRanges
...
format.sites4diffTest <- function(wins, sites, chr) {
  .list <- lapply(names(sites),
                  select.siteWins,
                  wins=wins,
                  sites=sites,
                  chr=chr)
  names(.list) <- names(sites)
                            ...
                            ...)
{
  ...
  rangeFun <- function(x) {
    ...
    ##s.id <- paste(unique(x[,index.col]), collapse="-", sep="")
